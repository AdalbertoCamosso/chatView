...
import { AddEstoque, AllEstoque, DeleEstoque } from "./controller";
  ...
  Object.values(item).some((val) => val.toString().toLowerCase().includes(query.toLowerCase()) )
    ...
    return (
      ...
      <header className="flex w-[80%] flex-col p-2 max-sm:flex-col">
        <h1 className=" font-semibold text-green-400 text-2xl">{"#"}Estoque</h1>
        <div>
          <div>
            ...
            />
                ...
                save={
                  ...
                  handleSubmit(async(data)=>{ 
                    router.refresh()
                    console.log(data)
                    const data_= await AddEstoque({nome:data.nome})
                ...
                <Button className="bg-green-700 rounded-[8px] ml-2 p-5 text-white hover:bg-blue-900  max-sm:w-[50px] max-sm:text-[24px]">
                  ...
                  <p className="max-sm:hidden">Adicionar Estoque</p>
                </Button>
                } 
                Description="Adicionando Estoque"
                title='Estoque'
